...
export const metadata: Metadata = {
  title: `Cours de guitare — ${siteConfig.name}`,
  description: CATEGORY_DESCRIPTIONS.cours,
  openGraph: {
    title:       `Cours de guitare — ${siteConfig.name}`,
    description: CATEGORY_DESCRIPTIONS.cours,
  },
...
export default async function CoursPage({ searchParams }: PageProps) {
  ...
  const all = getPostsByCategory("cours");
  ...
  const allStyles   = getAllStyles("cours");
  const styleCounts = getStyleCounts("cours");
  ...
  const meta = CATEGORY_META.cours;
  ...
  return (
    <main>
      <PageHero
        ...
        stats={[
          { value: all.length,  label: "cours" },
          { value: freeCount,   label: "gratuits" },
          { value: all.filter((i) => i.youtube).length, label: "avec vidéo" },
        ]}
      />
